...
public class TaskAcharUsuarioPorEMailRecuperarSenha extends AsyncTask<String, String, String> {
	...
	protected TelaRecuperarSenha telaRecuperarSenha;
	
	protected String email;

	public TaskAcharUsuarioPorEMailRecuperarSenha(ProgressDialog loadingDaTela, TelaRecuperarSenha telaRecuperarSenha)
	{
	...
	@Override
	protected String doInBackground(String... email_usuario) {
		this.email = email_usuario[0];
		
		
		
		
		String url_select = "http://server.sumosensei.pairg.dimap.ufrn.br/app/pegarusuarioporemail_semsenha.php";//android nao aceita localhost, tem de ser seu IP
		 ...
		 try {
	            ...
	            if(jArray.length() > 0)
	            {
	            	for(int i=0; i < jArray.length(); i++) {
		                ...
		                if(emailUsuario.compareTo(this.email) == 0)
		                {
		                	SingletonGuardaUsernameUsadoNoLogin guardaUsername = SingletonGuardaUsernameUsadoNoLogin.getInstance();
		            		guardaUsername.setEmailJogador(emailUsuario, this.telaRecuperarSenha.getApplicationContext());
		            		guardaUsername.setNomeJogador(nomeDeUsuario, this.telaRecuperarSenha.getApplicationContext());
		            		guardaUsername.setSenhaJogador(senhaUsuario, this.telaRecuperarSenha.getApplicationContext());
		                	this.popupDeProgresso.dismiss();
		            		this.telaRecuperarSenha.mandarMensagemRecuperaSenha();
